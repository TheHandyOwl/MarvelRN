export const colorPrimario = 'rgb(118, 25, 113)'
export const colorSecundario = 'rgb(180, 70, 176)'
export const colorTerciario = 'rgb(218, 176, 217)'
export const colorTerciarioTransparente = 'rgba(218, 176, 217,0.8)'
export const colorCuaternario = 'rgb(254, 245, 222)'

export const black = 'black'
export const blue = 'blue'
export const grey = 'grey'
export const green = 'green'
export const pink = 'pink'
export const red = 'red'
export const yellow = 'yellow'
export const white = 'white'

export const navBar = colorPrimario
export const navBarText = colorCuaternario
export const background = colorCuaternario
export const sectionListTitleBackground = colorSecundario
export const labelInput = colorPrimario
export const textInput = colorPrimario
export const darkButton = colorSecundario
export const lightButton = colorTerciario
export const transparentButton = colorTerciarioTransparente
export const darkTextButton = colorPrimario
export const lightTextButton = colorCuaternario
export const imageBackground = colorTerciario
export const whiteShadowBG = 'rgba(255,255,255,0.2)'
export const blackShadowBG = 'rgba(0,0,0,0.2)'
